...
| Feature                                     | Status      | Evidence/Notes                                                                                                                                                                                                                                                                                                               |
| ------------------------------------------- | ----------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Browse / search / view listings             | PARTIAL     | No search/filter in the module; "My Internships" tab lists _engagements_ (`tabs/MyInternshipsTab.tsx:6`), not open listings. Browse only via Feed (`Student/Feed/index.tsx:294-313`), and detail page loads the **engagement** endpoint (`InternshipDetail.tsx:43` → `getOne` on engagements) so public listings likely 404. |
...
- **Mock fallbacks masking failures**: `EventsService.list/getOne/myTickets` and `FeedService.list` silently return mock data on _any_ fetch failure, so a down/broken backend would be invisible to QA.
